...
class FonBased(Diagnostic):
    ...
    def FonTask(self, mol, program, method, basis=None):
        ...
        if program == "terachem":
            ...
            additional_keywords = {"gpus": "1",
                                   ...
                                   "scf": "diis+a",
                                   "levelshift": "yes",
                                   "levelshiftvala":self.levelshiftvala,
                                   "levelshiftvalb":self.levelshiftvalb,
                                   "convthre": "1e-6",
                                   "precision": "double",
                                   "units": "bohr",
                                   "fon": "yes",
    ...
    def computeFon(self, method, basis=None):
        ...
        print("self.molecule: ", self.molecule)
        ...
        if self.record:
            ...
            if molecule_result.success:
                fj = json.load(open(filename))
                fileout = self.rundir + "/" + self.diagnostic_type + "_" \
                          + self.molname + "_" + method + "_" + "whole" + ".out"
                with open(fileout, "w") as fo:
                    for line in fj["stdout"].split('\n'):
                        fo.write(line + '\n')
        if not molecule_result.success:
            print("!!!!:", molecule_result.__dict__.keys())
    ...
    def getEntanglement(self, fons, norbs, restricted):
        if not restricted:
            ...
            print(
                "For unrestricted entanglement calculations, Unrestricted Natural Orbitals are needed but not supported in TreaChem yet")
            return -1
